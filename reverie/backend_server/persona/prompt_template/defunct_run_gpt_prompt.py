...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False):
    ...
    #     with her customers on February 14th, 2023 at 5pm. She is gathering party material, and is telling everyone
    ...
    ## create_prompt_input()은 내부 함수임
    prompt_input = create_prompt_input(persona, test_input)  ## test_input = None임
...
def run_gpt_prompt_action_arena(
    ...
    x = [
        i.strip() for i in persona.s_mem.get_str_accessible_sector_arenas(y).split(",")
    ]
    if output not in x:
        output = random.choice(x)
